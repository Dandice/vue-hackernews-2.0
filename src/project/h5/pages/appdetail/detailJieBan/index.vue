...
<script>
    ...
    import {loadGA, openApp, _Platform, _getQueryString} from '../../../../utils/util'
    ...
    export default {
        ...
        data () {
            ...
            const authorid = url.indexOf('authorid') > -1 ? uid : null;
            const _ChaPage = (page) => {//
                let cpLoa = url.split('?')
                ...
                if (cpSpi[2]) {
                    if (authorid) {
                        return authorid === 'all' ? newUrl : `${newUrl}?authorid=${authorid}`
                    } else if (cpLoa[1]) {
                        return `${newUrl}?${cpLoa[1]}`
                    } else {
                        return newUrl
                    }
                }
            };
            let authoronlyUrl = `${_ChaPage(1)}?authorid=${uid}`;
            ...
            return {
                ...
                `<tid>${id}</tid><author>${author.username}</author><nexturl>${nextUrl}</nexturl><companytid id=${id}></companytid>` +
                `<updatedtime>${last_post_time}</updatedtime><elevator>${elevator.length ? 1 : 0}</elevator>` +
                `<timeago>2015-10-23</timeago><photo>${sharePic}</photo><iscompany id="together"></iscompany><companyuid id=${uid}></companyuid>`,
                //appData: '',
                artId: id
            }
        },
